...
void combinePictureStruct(Picture* pictures, Picture* picturesForMaster, Picture* picturesForWorker, int numPictures, int numPicturesForWorker, int numPicturesForMaster)
{
	...
	for (int i = 0; i < numPictures; i++)
	{
		...
		{
			if(pictures[i].picID == picturesForMaster[j].picID)
			{
				pictures[i].numObj = picturesForMaster[j].numObj;
				for (int l = 0; l < NUM_OF_DIFF_OBJ; l++)
					pictures[i].foundObj[l] = picturesForMaster[j].foundObj[l];
			
				for (int o = 0; o < NUM_OF_DIFF_OBJ; o++)
					for (int p = 0; p < 2; p++)
						pictures[i].positions[o][p] = picturesForMaster[j].positions[o][p];
				j++;
			}
		}	
		
		if(k != numPicturesForWorker)
		{
			if(pictures[i].picID == picturesForWorker[k].picID)
			{
				pictures[i].numObj = picturesForWorker[k].numObj;
				for (int l = 0; l < NUM_OF_DIFF_OBJ; l++)
					pictures[i].foundObj[l] = picturesForWorker[k].foundObj[l];
				
				for (int o = 0; o < NUM_OF_DIFF_OBJ; o++)
					for (int p = 0; p < 2; p++)
						pictures[i].positions[o][p] = picturesForWorker[k].positions[o][p];
				k++;
			}
		}
	}
